...
const TAB_SIZES = {
  ...
  tools: { width: EXPANDED_WIDTH, panelHeight: EXPANDED_PANEL_HEIGHT },
...
const toolPlatform = createToolPlatform({
  rootDir: path.join(app.getPath('userData'), 'tools'),
  hostVersion: app.getVersion(),
});
const nativeModule = createNativeModuleManager({
...
const AGENT_TASK_WATCH_INTERVAL_MS = 2500;
...
let agentTaskWatchTimer = null;
let watchedAgentTaskId = null;
const recentAgentStateSignals = new Set();
let approvedAgentWorkspacePath = null;
...
function resolveCodexBinary() {
  const configured = String(process.env.CLOCKOUT_CODEX_BIN || '').trim();
  const candidates = [
    configured,
    '/Applications/ChatGPT.app/Contents/Resources/codex',
    path.join(app.getPath('home'), '.codex', 'bin', 'codex'),
...
function ensureAgentService() {
  ...
  const servicePath = app.isPackaged
    ? path.join(process.resourcesPath, 'agent-service-v2', 'server.mjs')
    : path.resolve(__dirname, '..', '..', 'packages', 'agent-service-v2', 'server.mjs');
  agentService = createAgentServiceManager({
    executable: process.execPath,
    servicePath,
    dataDir: path.join(app.getPath('userData'), 'tasks'),
    codexBinary: resolveCodexBinary(),
  });
  return agentService;
}

function readAgentReminderSettings() {
  return normalizeReminderSettings(
    readJsonFile(getJsonSettingsPath(AGENT_REMINDER_SETTINGS_FILE))
  );
}

function publicAgentReminderSettings() {
  const settings = readAgentReminderSettings();
  return { ...settings, nextAt: nextReminderAt(settings, Date.now()) };
}

function saveAgentReminderSettings(settings) {
  const normalized = normalizeReminderSettings(settings);
  if (!writeJsonFile(getJsonSettingsPath(AGENT_REMINDER_SETTINGS_FILE), normalized)) {
    return false;
  }
  scheduleAgentReminder();
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('agent:reminder-settings-changed', publicAgentReminderSettings());
  }
  return true;
}

function clearAgentReminderTimer() {
  if (agentReminderTimer) clearTimeout(agentReminderTimer);
  agentReminderTimer = null;
}

function fireAgentReminder() {
  const firedAt = Date.now();
  const settings = readAgentReminderSettings();
  enqueueTaskNotification({
    eventId: `agent-reminder-${firedAt}`,
    source: 'agent',
    taskId: 'agent-handoff',
    title: '快下班了，要不要现在交接？',
    detail: '打开灵动岛，确认今天剩下的工作',
    project: 'IRiXi的小工具库',
    completedAt: firedAt,
  });
  openRendererPanel('app:open-agent');
  setTimeout(() => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('agent:reminder-fired', {
        firedAt,
...
function scheduleAgentReminder() {
  clearAgentReminderTimer();
  const nextAt = nextReminderAt(readAgentReminderSettings(), Date.now());
  if (!nextAt) return;
  agentReminderTimer = setTimeout(fireAgentReminder, Math.max(250, nextAt - Date.now()));
}
...
function refreshTrayMenu() {
  ...
  const settings = readAppSettings();
  const featureLabels = { todo: '待办', notes: '笔记', recordings: '录制', clip: '剪贴板' };
...
ipcMain.handle('settings:set-feature', (event, payload) => {
  if (!['todo', 'notes', 'recordings', 'clip'].includes(payload && payload.featureId)) {
...
function builtInToolList() {
  ...
  return [
    { id: 'builtin.todo', name: '待办', description: '四象限任务和到期提醒', tab: 'todo', featureId: 'todo', enabled: settings.features.todo !== false },
    { id: 'builtin.notes', name: '笔记', description: '随手记录和笔记资料库', tab: 'notes', featureId: 'notes', enabled: settings.features.notes !== false },
    { id: 'builtin.recordings', name: '录音', description: '录音、保存和语音转写', tab: 'recordings', featureId: 'recordings', enabled: settings.features.recordings !== false },
    { id: 'builtin.pomodoro', name: '番茄钟', description: '首页里的专注计时器', tab: 'home', featureId: null, enabled: true },
    { id: 'builtin.clip', name: '剪贴板', description: '可选开启的本机剪贴历史', tab: 'clip', featureId: 'clip', enabled: settings.features.clip === true },
    {
      id: 'builtin.translation',
      name: '输入翻译',
      description: '输入互译、跨应用划译和英文朗读',
      tab: null,
      featureId: null,
      enabled: true,
      available: true,
      nativeAction: 'translation.input',
      nativeBackend: 'module',
      helperState: nativeModule.status().state,
    },
    {
      id: 'builtin.capture',
      name: '截图与识字',
      description: '截图、识别屏幕文字和翻译图片',
      tab: null,
      featureId: null,
      enabled: true,
      nativeAction: 'capture.area',
      nativeBackend: 'module',
      helperState: nativeModule.status().state,
    },
  ].map((tool) => ({
    ...
    author: 'IRiXi',
    version,
    capabilities: tool.nativeBackend === 'module' ? ['native-module'] : ['local'],
    configurable: Boolean(tool.featureId),
  }));
}

function toolResult(action) {
  try {
    return { ok: true, ...action() };
  } catch (error) {
    const known = error instanceof ToolPackageError;
    return {
      ok: false,
      error: known ? error.code : 'tool_error',
      message: known ? error.message : '工具操作没有完成，其他内容未受影响。',
    };
  }
}
...
function untrustedNativeModuleResult() {
  return { ok: false, error: 'untrusted_sender' };
}

ipcMain.handle('native-module:status', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return { ok: true, ...nativeModule.status() };
});

ipcMain.handle('native-module:test-window-open', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.openTestWindow();
});

ipcMain.handle('native-module:test-window-close', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.closeTestWindow();
});

ipcMain.handle('native-module:capture-area', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.startAreaCapture();
});

ipcMain.handle('native-module:capture-window', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.startWindowCapture();
});

ipcMain.handle('native-module:capture-fullscreen', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.startFullscreenCapture();
});

ipcMain.handle('native-module:capture-ocr', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.startOCRCapture();
});

ipcMain.handle('native-module:capture-translate', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.startImageTranslationCapture();
});

ipcMain.handle('native-module:translation-input', (event, partner) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.openInputTranslation(partner);
});

ipcMain.handle('native-module:translation-selection', (event) => {
  if (!isTrustedMainWindowSender(event)) return untrustedNativeModuleResult();
  return nativeModule.translateCurrentSelection();
});

function readBundledCounter() {
  return fs.readFileSync(BUNDLED_COUNTER_PATH);
}

ipcMain.handle('tools:list', () => toolResult(() => {
  const installed = toolPlatform.list();
  const sampleInstalled = installed.some((tool) => tool.id === 'sample.counter');
  return {
    items: [...builtInToolList(), ...installed],
    sampleAvailable: !sampleInstalled,
  };
}));

ipcMain.handle('tools:preview-sample', () => toolResult(() => ({
  preview: toolPlatform.preview(readBundledCounter()),
})));

ipcMain.handle('tools:install-sample', () => toolResult(() => ({
  tool: toolPlatform.install(readBundledCounter(), 'bundled'),
})));

ipcMain.handle('tools:set-enabled', (event, payload) => toolResult(() => ({
  tool: toolPlatform.setEnabled(String(payload && payload.id || ''), payload && payload.enabled),
})));

ipcMain.handle('tools:get-state', (event, toolId) => toolResult(() => ({
  state: toolPlatform.getState(String(toolId || '')),
})));

ipcMain.handle('tools:run-action', (event, payload) => toolResult(() => ({
  state: toolPlatform.runAction(
    String(payload && payload.id || ''),
    String(payload && payload.actionId || '')
  ),
})));

ipcMain.handle('tools:uninstall', (event, payload) => toolResult(() => ({
  result: toolPlatform.uninstall(
    String(payload && payload.id || ''),
    payload && payload.keepData
  ),
})));
...
function assertAgentRenderer(event) {
  throw new Error('旧 Agent 功能已停用。');
}

async function runningAgentService() {
  const service = ensureAgentService();
  const state = service.status().state;
  if (state === 'stopped' || state === 'starting') await service.start();
  if (service.status().state === 'failed') {
    throw new Error(service.status().error || 'Agent 服务没有启动。');
  }
  return service;
}

function stopAgentTaskWatch(taskId = null) {
  if (taskId && watchedAgentTaskId && watchedAgentTaskId !== taskId) return;
  if (agentTaskWatchTimer) clearTimeout(agentTaskWatchTimer);
  agentTaskWatchTimer = null;
  watchedAgentTaskId = null;
}

function rememberAgentStateSignal(signal) {
  if (recentAgentStateSignals.has(signal)) return false;
  recentAgentStateSignals.add(signal);
  if (recentAgentStateSignals.size > 50) {
    recentAgentStateSignals.delete(recentAgentStateSignals.values().next().value);
  }
  return true;
}

function notifyAgentTaskState(task) {
  const decision = classifyAgentTaskForWatch(task);
  if (decision.action !== 'notify') return { ok: false, ignored: true };
  stopAgentTaskWatch(decision.id);
  if (!rememberAgentStateSignal(decision.signal)) return { ok: true, duplicate: true };
  const copy = {
    waiting_user: ['Agent 需要你确认一件事', '打开灵动岛回复后，它会继续'],
    succeeded: ['任务已经完成，可以验收', '检查改动和验证结果'],
    partial: ['任务只完成了一部分', '看看还需要你决定什么'],
    failed: ['任务遇到问题，已经停下', '打开灵动岛查看原因'],
    cancelled: ['任务已经安全停止', '打开灵动岛查看当前结果'],
  }[decision.status];
  const workspaceName = String(
    task.workspaceName || (task.workspacePath && path.basename(task.workspacePath)) || 'IRiXi的小工具库'
  ).slice(0, 120);
  enqueueTaskNotification({
    eventId: `agent-${decision.signal}`,
    source: 'agent',
    taskId: decision.signal,
    title: copy[0],
    detail: copy[1],
...
function scheduleAgentTaskWatch() {
  if (isQuitting || !watchedAgentTaskId || agentTaskWatchTimer) return;
  agentTaskWatchTimer = setTimeout(() => {
    agentTaskWatchTimer = null;
    void pollAgentTaskWatch();
  }, AGENT_TASK_WATCH_INTERVAL_MS);
}

function observeAgentTask(task) {
  const decision = classifyAgentTaskForWatch(task);
  if (decision.action === 'watch') {
    if (watchedAgentTaskId !== decision.id) stopAgentTaskWatch();
    watchedAgentTaskId = decision.id;
    scheduleAgentTaskWatch();
    return;
  }
  if (!watchedAgentTaskId || watchedAgentTaskId !== decision.id) return;
  if (decision.action === 'notify') notifyAgentTaskState(task);
  else stopAgentTaskWatch(decision.id);
}

async function pollAgentTaskWatch() {
  const taskId = watchedAgentTaskId;
  if (!taskId) return;
  const service = agentService;
  if (!service || service.status().state !== 'running') {
    stopAgentTaskWatch(taskId);
    return;
  }
  try {
    const response = await service.request('get', { id: taskId });
    if (watchedAgentTaskId === taskId) observeAgentTask(response && response.task);
  } catch (error) {
    stopAgentTaskWatch(taskId);
  }
}

async function restoreAgentTaskWatch() {
  const service = agentService;
  if (!service || service.status().state !== 'running') return;
  try {
    const response = await service.request('current');
    const decision = classifyAgentTaskForWatch(response && response.task);
    if (decision.action === 'watch') observeAgentTask(response.task);
    else stopAgentTaskWatch();
  } catch (error) {
    stopAgentTaskWatch();
  }
}

function trackAgentResponse(response) {
  observeAgentTask(response && response.task);
  return response;
}

ipcMain.handle('agent:status', (event) => {
  assertAgentRenderer(event);
  return {
    service: ensureAgentService().status(),
    reminder: publicAgentReminderSettings(),
  };
});
...
ipcMain.handle('agent:restart', async (event) => {
  assertAgentRenderer(event);
  stopAgentTaskWatch();
  await ensureAgentService().restart();
  await restoreAgentTaskWatch();
  return ensureAgentService().status();
});

ipcMain.handle('agent:current', async (event) => {
  assertAgentRenderer(event);
  return trackAgentResponse(await (await runningAgentService()).request('current'));
});

ipcMain.handle('agent:create', async (event, payload) => {
  assertAgentRenderer(event);
  const requestedPath = String(payload && payload.workspacePath || '');
  if (!approvedAgentWorkspacePath || requestedPath !== approvedAgentWorkspacePath) {
    throw new Error('请先用系统文件夹选择器确认本次工作区。');
  }
  const task = trackAgentResponse(await (await runningAgentService()).request('create', {
    workspacePath: approvedAgentWorkspacePath,
    goal: payload && payload.goal,
  }));
  approvedAgentWorkspacePath = null;
  return task;
});

ipcMain.handle('agent:get', async (event, id) => {
  assertAgentRenderer(event);
  return trackAgentResponse(await (await runningAgentService()).request('get', { id }));
});

ipcMain.handle('agent:message', async (event, payload) => {
  assertAgentRenderer(event);
  return trackAgentResponse(await (await runningAgentService()).request('message', payload));
});

for (const action of ['confirm', 'cancel', 'rollback', 'accept', 'continue', 'end']) {
  ipcMain.handle(`agent:${action}`, async (event, payload) => {
    assertAgentRenderer(event);
    return trackAgentResponse(await (await runningAgentService()).request(action, payload));
  });
}

ipcMain.handle('agent:choose-workspace', async (event) => {
  assertAgentRenderer(event);
  const result = await showOwnedOpenDialog({
    title: '选择本次 Agent 可以处理的文件夹',
    properties: ['openDirectory'],
  });
  const selected = !result.canceled && Array.isArray(result.filePaths) && result.filePaths[0];
  if (!selected) return { ok: true, canceled: true };
  try {
    approvedAgentWorkspacePath = fs.realpathSync.native(selected);
    return { ok: true, path: approvedAgentWorkspacePath };
  } catch (error) {
    approvedAgentWorkspacePath = null;
    throw new Error('无法读取这个文件夹，请重新选择。');
  }
});

ipcMain.handle('agent:reminder:get', (event) => {
  assertAgentRenderer(event);
  return publicAgentReminderSettings();
});

ipcMain.handle('agent:reminder:set', (event, payload) => {
  assertAgentRenderer(event);
  const current = readAgentReminderSettings();
  const next = normalizeReminderSettings({
    ...current,
    enabled: payload && payload.enabled,
    endTime: payload && payload.endTime,
    leadMinutes: payload && payload.leadMinutes,
    skippedDate: null,
    snoozedUntil: null,
  });
  return saveAgentReminderSettings(next)
    ? { ok: true, reminder: publicAgentReminderSettings() }
    : { ok: false, error: 'save_failed' };
});

ipcMain.handle('agent:reminder:snooze', (event, minutes) => {
  assertAgentRenderer(event);
  const safeMinutes = Math.max(5, Math.min(120, Math.round(Number(minutes) || 15)));
  const next = {
    ...readAgentReminderSettings(),
    enabled: true,
    skippedDate: null,
    snoozedUntil: Date.now() + safeMinutes * 60 * 1000,
  };
  return saveAgentReminderSettings(next)
    ? { ok: true, reminder: publicAgentReminderSettings() }
    : { ok: false, error: 'save_failed' };
...
ipcMain.handle('agent:reminder:skip-today', (event) => {
  assertAgentRenderer(event);
  const next = {
    ...readAgentReminderSettings(),
    skippedDate: localDateKey(Date.now()),
    snoozedUntil: null,
  };
  return saveAgentReminderSettings(next)
    ? { ok: true, reminder: publicAgentReminderSettings() }
...
ipcMain.handle('agent:open-remote-settings', async (event) => {
  assertAgentRenderer(event);
  try {
    await shell.openExternal('codex://settings/connections/computer');
    return true;
  } catch (error) {
    return false;
  }
...
ipcMain.handle('agent:signal-state', (event, payload) => {
  assertAgentRenderer(event);
  return notifyAgentTaskState(payload);
});
...
async function runW19ScreenPermissionTest(startupStatus) {
  const before = startupStatus?.screenPermission || 'unknown';
  let requestReturned = null;
  if (startupStatus?.ok === true) {
    const requestResult = nativeModule.requestScreenPermission();
    requestReturned = requestResult.ok === true ? requestResult.requested === true : null;
  }
  await new Promise((resolve) => setTimeout(resolve, 750));
  const refreshed = nativeModule.refreshScreenPermission();
  const result = {
    bundleId: nativeModule.status().bundleId,
    before,
    requestReturned,
    after: refreshed.screenPermission,
  };
  const resultPath = `/private/tmp/irixi-w19-screen-permission-result-${process.pid}.json`;
  try {
    ...
    console.log(`[w19-screen-permission] ${resultPath}`);
  } catch (_error) {
    console.error('[w19-screen-permission] result_write_failed');
  }
}
...
app.on('will-quit', () => {
  ...
  if (agentService) agentService.stop();
