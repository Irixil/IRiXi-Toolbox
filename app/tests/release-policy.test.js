...
const upstreamPath = path.join(projectRoot, 'UPSTREAM.md');
...
test('the fork preserves upstream attribution and records release blockers', () => {
  const upstream = fs.readFileSync(upstreamPath, 'utf8');
  const notices = fs.readFileSync(noticesPath, 'utf8');

  assert.match(upstream, /xiaopu-ai\/TO-DO-Panel/);
  assert.match(upstream, /e69cccaa27dd13333f6eeee483ac9187bade2299/);
  assert.match(notices, /renderer\/effects\.js/);
  assert.match(notices, /发布前/);
});
