...
- 移除无法可靠控制网易云的进度条与歌词点击跳转，避免出现“界面动了、音乐没动”的假交互。
- 恢复当前歌曲封面、歌名、歌手、专辑和歌词；歌词根据播放状态自动高亮并滚动，播放控制仍保留播放、暂停、上一首和下一首。
- 当前歌曲与播放时间改以 macOS 正在播放信息为准；本地缓存只有在歌名和歌手匹配时才用于补充封面与歌词，切歌期间不再短暂显示上一首歌。

### 改进

- 修复在页签和同一应用的原生工具窗口之间切换时，主面板偶发误收起的问题；截图入口仍会在开始捕获前主动收起面板。
- 去除网易云音乐大窗沿用通用卡片样式产生的下边与右边轮廓，歌词布局与对齐方式保持不变。
- 统一的翻译窗口新增不联网的本机详细释义，可对选中的单词或短语查词典；同时增加用户主动触发的 OpenAI 联网解释，支持上下文和可点击的网页来源。API 密钥只保存在 macOS 系统钥匙串，断网或 AI 失败不影响本机翻译与朗读。
- 截图快捷键改由进程内原生模块直接监听，不再与文字翻译串线；启动失败时显示准确原因，不再误跳到工具页。
- 翻译与朗读窗口会显示在灵动岛上方，修复截图翻译窗被主面板遮住、看起来没有弹出的问题。
- 快捷键设置页区分“被其他软件占用”和“截图组件未连接”，不再用错误的已启用状态误导用户。
- 恢复 Snaploom 的默认区域截图快捷键 `⌘⇧X`：无需先打开小工具库即可截图；设置页可修改并会显示快捷键占用状态。
- 截图后选择“翻译文字”会直接进入统一的“IRiXi 翻译与朗读”窗口，自动填入识别结果并翻译，可继续编辑、切换语言、复制译文或朗读英文。
- 删除截图翻译内部重复的翻译桥接和简化结果窗口，保持单一应用、单一权限身份与本机 Apple 翻译。
