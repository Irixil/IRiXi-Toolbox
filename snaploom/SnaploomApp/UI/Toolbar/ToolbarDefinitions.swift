...
enum ToolbarCustomAction: Int {
    ...
    static var bottomToolbarActions: [ToolbarCustomAction] {
        #if IRIXI_HELPER
        // Stage 1 keeps the proven annotation tools and omits unrelated image
        // effects until they have their own accepted entry and desktop checks.
        return []
        #else
    ...
    static var rightToolbarActions: [ToolbarCustomAction] {
        #if IRIXI_HELPER
        // No upload/share, scroll capture or recording entry is exposed by the
        // embedded helper in this stage. Pin, OCR and local translation are the
        // only extra output actions beside copy/save/cancel/editor.
        return [.pin, .ocr, .translate]
        #else
