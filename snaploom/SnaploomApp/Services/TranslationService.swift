...
final class TranslationBridge: ObservableObject {
    ...
    func translate(
        ...
        // Language preparation can include a first-use local language-pack download.
        DispatchQueue.main.asyncAfter(deadline: .now() + 120) { [weak self] in
            ...
            self.cleanup()
            completion?(.failure(TranslationError.appleTranslation("Apple 本机翻译等待超时，请确认语言包可用后重试。")))
        }
    }
