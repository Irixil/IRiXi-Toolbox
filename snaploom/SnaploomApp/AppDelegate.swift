...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var nativeHelperServer: NativeHelperServer?
    private var nativeTranslationWindow: NativeInputTranslationWindowController?
    private let translationSelectionService = TranslationSelectionService()
    private let translationSelectionSourceTracker = TranslationSelectionSourceTracker()
    private let translationShortcutManager = TranslationShortcutManager()
    private var translationShortcutStartupMessage: String?
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        startNativeHelperMode()
        return

        /* The original standalone Snaploom launch path is intentionally kept
           below while its capture/editor modules are folded into the helper.
           It is unreachable in the IRiXi helper build and will be removed as
           duplicate product chrome before packaging. */
    ...
    private func startNativeHelperMode() {
        NSApp.setActivationPolicy(.accessory)

        if case .failure(let reason) = NativeHelperParentValidator.validateParent() {
            let message = Data("parent_validation:\(reason.rawValue)\n".utf8)
            try? FileHandle.standardError.write(contentsOf: message)
            NSApp.terminate(nil)
            return
        }

        nativeHelperServer = NativeHelperServer { [weak self] command, completion in
            DispatchQueue.main.async {
                ...
                case .health:
                    completion(true)
                case .translationInputOpen:
                    self.openNativeInputTranslation(partner: command.partner ?? "en")
                    completion(true)
                case .translationSelection:
                    let partner = command.partner ?? "en"
                    self.translateCurrentSelection(partner: partner, completion: completion)
                case .captureArea, .captureWindow:
                    completion(self.startNativeCapture(.area))
                case .captureFullscreen:
                    completion(self.startNativeCapture(.fullscreen))
                case .captureOCR:
                    completion(self.startNativeCapture(.ocr))
                case .captureTranslate:
                    completion(self.startNativeCapture(.translate(command.target)))
                }
            }
        }
        nativeHelperServer?.start()
        prepareNativeCaptureFeatures()
        translationSelectionSourceTracker.start()

        translationShortcutManager.onTrigger = { [weak self] in
        ...
        do {
            try translationShortcutManager.register(.load())
        } catch {
            translationShortcutStartupMessage = "划译快捷键暂时无法启用；仍可从 IRiXi 打开输入翻译"
        }
    }

    private enum NativeCaptureKind {
        case area
        case fullscreen
        case ocr
    ...
    private func prepareNativeCaptureFeatures() {
        TranslationService.provider = .apple
        appNapAssertion = ProcessInfo.processInfo.beginActivity(
            options: [.userInitiatedAllowingIdleSystemSleep],
            reason: "IRiXi native tool responsiveness"
        )
        _ = ClipboardBackingStore.directory
        LaunchCleanup.runAll()
        _ = ScreenshotHistory.shared
        ...
        NotificationCenter.default.addObserver(
            ...
            name: .init("Snaploom.pinFromHistory"),
            object: nil
        )
    ...
    private func startNativeCapture(_ kind: NativeCaptureKind) -> Bool {
        guard canStartCapture else { return false }
        guard PermissionOnboardingController.hasScreenRecordingPermission() else {
            // This runs only after the user explicitly chose a capture action.
            // The request makes the nested helper appear in macOS's Screen
            // Recording list; merely opening System Settings is not enough on
        ...
        case .translate(let target):
            beginCaptureTranslate(target: target, fromMenu: false)
        }
        return true
    }

    private func openNativeInputTranslation(partner: String) {
        ensureNativeTranslationWindow()
        nativeTranslationWindow?.show(partner: partner)
    }

    private func openNativeSelectionTranslation(_ text: String, partner: String) {
        ensureNativeTranslationWindow()
        nativeTranslationWindow?.showSelection(text, partner: partner)
    }

    private func openNativeSelectionError(
        _ error: TranslationSelectionError,
        partner: String
    ) {
        if error == .accessibilityPermission {
            translationSelectionService.requestAccessibilityPermission()
        }
        ensureNativeTranslationWindow()
        nativeTranslationWindow?.showSelectionError(error, partner: partner)
    }

    private func ensureNativeTranslationWindow() {
        guard nativeTranslationWindow == nil else { return }
        nativeTranslationWindow = NativeInputTranslationWindowController { [weak self] shortcut in
            guard let self else { return false }
            do {
                try self.translationShortcutManager.register(shortcut)
                self.translationShortcutStartupMessage = nil
                return true
            } catch {
                return false
            }
        }
        if let translationShortcutStartupMessage {
            nativeTranslationWindow?.showShortcutMessage(translationShortcutStartupMessage)
        }
    }
    ...
    private func translateCurrentSelection(
        ...
        let sourceApplication = translationSelectionSourceTracker.preferredSourceApplication()
        Task { @MainActor [weak self] in
            ...
            do {
                let text = try await self.translationSelectionService.captureSelection(
                    from: sourceApplication
                )
                self.openNativeSelectionTranslation(text, partner: partner)
            } catch let error as TranslationSelectionError {
                self.openNativeSelectionError(error, partner: partner)
            } catch {
                self.openNativeSelectionError(.noSelection, partner: partner)
            }
            completion?(true)
        }
    }
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        // The embedded helper has no standalone settings or product entry.
        // Reopening its bundle while IRiXi owns it must never reveal Snaploom UI.
        if nativeHelperServer != nil {
            return false
        }
